...
Generated Files
---------------
1. src/main/Foo.es6.js                   Source file for the new class
2. src/test/Foo-universal-test.es6.js    Basic unit tests to start you off
3. src/test/Foo-browser-test.es6.js      As above, for browsers only
4. src/test/Foo-nonbrowser-test.es6.js   As above, for Node.js only
5. src/demo/Foo-demo.es6.js              Usage example script
...
classes.forEach( name => {
    const path = `src/main/${name}.es6.js`
...
//// 2. src/test/Foo-universal-test.es6.js    Basic unit tests to start you off
classes.forEach( name => {
    const path = `src/test/${name}-universal-test.es6.js`
    if ( fs.existsSync(path) ) return console.warn(`${path} already exists`)
...
//// 3. src/test/Foo-browser-test.es6.js      As above, for browsers only
classes.forEach( name => {
    const path = `src/test/${name}-browser-test.es6.js`
    if ( fs.existsSync(path) ) return console.warn(`${path} already exists`)
...
//// 4. src/test/Foo-nonbrowser-test.es6.js   As above, for Node.js only
classes.forEach( name => {
    const path = `src/test/${name}-nonbrowser-test.es6.js`
    if ( fs.existsSync(path) ) return console.warn(`${path} already exists`)
...
//// 5. src/demo/Foo-demo.es6.js              Usage example script
classes.forEach( name => {
    const path = `src/demo/${name}-demo.es6.js`
    if ( fs.existsSync(path) ) return console.warn(`${path} already exists`)
      ...
      var i = ~~d.cookie.split('~')[1] // 0 - 3
        ...
        , s = // src values
              (1 == i) ? [ // Production Minified ES5
                ...
                , f + 'main/${projectLC}.es5.min.js'
                , f + 'demo/${nameLC}-demo.es5.js' // no need to minify
              ]
            : (2 == i) ? [ // Production ES6
                  f + 'main/${projectLC}.es6.js'
                , f + 'demo/${nameLC}-demo.es6.js'
              ]
            : (3 == i) ? [ // Development ES6
                  f + 'main/App.es6.js'
                , f + 'main/@todo.es6.js'
